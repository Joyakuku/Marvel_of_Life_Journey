 ...
 * 每道题包含：id、type、title、options、weight、scoreMap、explanation、section
 */
const QUESTIONS = [
  // 一、基本健康状况（44分）
  {
  ...
  },
  {
    ...
    weight: 8,
    // 将最高分与权重对齐（原为 B:6，导致章节合计仅42分，无法达到100满分）
    scoreMap: { A: 2, B: 8 },
  ...
  },
  {
    ...
    weight: 5,
    // 将最高分与权重对齐（原为 C:4，导致知识认知章节合计仅24分，无法达到100满分）
...
function getAllQuestions() {
  logger.debug('Questions', '获取题库', { count: QUESTIONS.length })
  return QUESTIONS
}
...
function getQuestionById(id) {
  ...
  if (!question) {
    logger.warn('Questions', '题目未找到', { id })
  }
...
/**
 * 计算答题结果与总分
 * @param {Object} answers 答题结果对象，格式：{ questionId: answer }
 * @returns {Object} 计算结果：{ totalScore, maxScore, level, description, details }
 */
function calculateScore(answers) {
  logger.info('Questions', '开始计算得分', { answerCount: Object.keys(answers).length })
  ...
  // 确定分数等级
  const level = SCORE_LEVELS.find(l => totalScore >= l.min && totalScore <= l.max) || SCORE_LEVELS[SCORE_LEVELS.length - 1]
  ...
  logger.info('Questions', '计分完成', { 
    totalScore, 
    maxScore, 
    level: level.level,
    answeredCount: details.filter(d => d.score > 0).length
  })
